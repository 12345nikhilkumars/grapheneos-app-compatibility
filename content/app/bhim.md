...
alternatives:
  - kind: app
    ...
    limitation: "Different app, different operator, and its own setup requirements. It also fails for some people — its entry records the conflict rather than resolving it."
...
- **October 2025, later.** Version `253000082` starts refusing to run with developer options enabled — a build-property check, and one that forces a reboot to work around.
...
The **onboarding failure** — "we are unable to onboard you at this time" — happens after that check is satisfied, and looks like server-side enforcement. That is consistent with the RBI standards reply: the decision is not being made on the phone.
